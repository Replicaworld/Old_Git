...
# In[9]:
print('Starting of NewsScore CV Incremental Update')
import logging
import datetime
import time
import os
...
print(os.getcwd())
if not os.path.exists('/home/Gourav/CV/logs/'):
    os.makedirs('/home/Gourav/CV/logs/')
...
hdlr = logging.FileHandler(
    '/home/Gourav/CV/logs/' + str(datetime.datetime.today().date()) + '.log')
...
gfv=spark.read.parquet("gs://pvtrough_asia_south1/tf_idf/devices_fulfilling_criteria")
df_temp=gfv.filter((gfv.devicehashcode< 81) & (gfv.devicehashcode > 70)).select(['deviceid']).distinct()
...
trialDB = mongoClient['trialDB']
deviceVectorCollection = trialDB['deviceVectors_CV']
deviceClusterCollection = trialDB['notiDeviceClusters_CV']

newsVectorCollection = trialDB['newsVectors_CV']
newsTSpentCollection = trialDB['newsTSpent_CV']
newsScoreCollection = trialDB['newsScores_CV']
deviceNotiClustersCollection = trialDB['notiDeviceClusters_CV']
