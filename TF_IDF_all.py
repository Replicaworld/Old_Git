...
# In[158]:
print('Starting of TF_IDF Clustering Update')
import logging
import datetime
import time
import os
...
suffixes = ['TF_IDF_High','TF_IDF_Low','TF_IDF_Med']
for suffix in suffixes:
    start_time=time.time()
    Log("Training for %s"%suffix)
    import redis
    r_conn1 = redis.StrictRedis('172.16.11.60', port=6379, db=1)

    lda_usrs=pd.DataFrame(list(map(lambda x:x.decode("utf-8"),list(r_conn1.smembers(suffix)))),columns=['deviceid'])
    ...
    min_device_in_cluster_threshold = 1000
    ...
    print(l_df.cluster.value_counts())
    print(to_be_merged_clusters)
    
    for index, row in tqdm(small_df.iterrows()):
        d = row['deviceId']
        arr = list(topicDf[topicDf.deviceId == d].values[0][1])
    #     print(d, arr)
        temp = arr.copy()
        new_label = row['cluster']
        i = 1
        while new_label in to_be_merged_clusters and i < 10:
            new_label = arr.index(sorted(temp, reverse = True)[i])
    #         print("ch: ",new_label)
            i += 1
        labels[d] = new_label
        
    Log("Updated labels for low device containing clusters")
    l_df = pd.DataFrame(labels.items(), columns = ['deviceId', 'cluster'])
    print(l_df.cluster.value_counts())
    ...
    insertNewsScoreInMongo(newsTSpentMap_filter)
    insert_time = (time.time() - insert_start)
