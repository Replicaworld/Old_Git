...
log_path = '/home/DS_Live_Trials/NewsScore_logs/'
print(os.getcwd())
...
for suffix in suffixes:
    ...
    def return_valid_paths(paths):
        valid = []
        for p in paths.split(","):
    #         print(p)
            try:
                r = subprocess.check_output("gsutil ls -R {0}".format(p), shell=True)
                valid.append(str(p))
            except:
                continue
    ...
    valid_paths = return_valid_paths(path)
    if len(valid_paths) == 0:
        r = process_raw_data(path)
    valid_paths = ','.join(valid_paths)
    
    view_data = process_raw_data(valid_paths)
    Log("View data Loaded %s"%valid_paths)
    ...
    for data_df in data_chunks:
        ...
        try:
            ...
            newsTSpentMap_filtered = defaultdict(lambda: defaultdict(lambda: (7, 1)))
            for hid, cmap in newsTSpentMap.items():
                for cluster, tvalues in cmap.items():
                    if tvalues[1] >= min_read_event_threshold: 
                        newsTSpentMap_filtered[hid][cluster] = tvalues

            Log(suffix)
            Log(len(newsTSpentMap))
            Log(len(newsTSpentMap_filter))
            
            insertNewsTSpentInMongo(newsTSpentMap_filtered)
            insertNewsScoreInMongo(newsTSpentMap_filtered)
