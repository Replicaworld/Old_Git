...
log_path = '/home/DS_Live_Trials/Roberta_Base_Clustering_logs/'
print(os.getcwd())
...
grouped_df = df_joined.groupBy("deviceId").agg(*[
    expr(f"avg(weighted_embedding[{i}]) as avg_{i}") for i in range(embedding_length)
])



# assuming your dataframe is called grouped_df
avg_cols = [col(f"avg_{i}") for i in range(embedding_length)]
...
def calculate_similarity(device_vector, cluster_center):
    return 1 - distance.cosine(device_vector, cluster_center)

def find_best_cluster(device_vector, cluster_centers):
    similarity_scores = [calculate_similarity(device_vector, center) for center in cluster_centers]
#     best_cluster_index = max(enumerate(similarity_scores), key=lambda x: x[1])[0]
...
topicDf = topicDf = df.toPandas()

l_df = pd.DataFrame(labels.items(), columns = ['deviceId', 'cluster'])
min_device_in_cluster_threshold = 200
small_df = l_df.groupby("cluster").filter(lambda x: len(x) < min_device_in_cluster_threshold)
to_be_merged_clusters = list(small_df.cluster.unique())
print(to_be_merged_clusters, small_df.shape[0])
not_to_be_merged_clusters = list(set(l_df.cluster.unique()) - set(to_be_merged_clusters))
with tqdm(total=small_df.shape[0]) as pbar:
    for index, row in small_df.iterrows():
        d = row['deviceId']
        old_label = row['cluster']
        new_label = old_label
        if old_label not in to_be_merged_clusters:
            pbar.update(1)
            continue
        arr = list(topicDf[topicDf.deviceId == d].values[0][1])
        similarity_scores = list(find_best_cluster(arr,cluster_centers))
        similarity_scores_sorted = sorted(similarity_scores, reverse = True)
        
        i = 0
        while new_label not in not_to_be_merged_clusters and i < 200:
            new_label = similarity_scores.index(similarity_scores_sorted[i])
    #         print("ch: ",new_label)
            i += 1
...
l_df = pd.DataFrame(labels.items(), columns = ['deviceId', 'cluster'])
Log("New Cluster Device Counts after merging :")
Log(l_df.cluster.value_counts())
...
def insertNewsTScoreInMongo(newsTSpentMap):
    for hashId in newsTSpentMap:
        ...
        scoreData = {"$set" : {"tSpent" : data, "updatedAt" : datetime.datetime.now()}, "$setOnInsert": {"createdAt": datetime.datetime.now()}}
...
for cluster in newClusterWiseDevices:
    for deviceId in tqdm(newClusterWiseDevices[cluster]):
        updatedDeviceClusters[deviceId] = labels[deviceId]
...
suffix = trial_algo_suffix
if 'low' in suffix.lower():
    min_read_event_threshold = 10
elif 'med' in suffix.lower():
    min_read_event_threshold = 30
else:
    min_read_event_threshold = 50


newsTSpentMap_filter = defaultdict(lambda: defaultdict(lambda: (7, 1)))
for v in tqdm(newsClusterMeanDf.values):
    if v[3]>=min_read_event_threshold:
        newsTSpentMap_filter[v[0]][v[1]] = (v[2], v[3])


Log(suffix)
Log(len(newsTSpentMap))
Log(len(newsTSpentMap_filter))
...
insert_start = time.time()
insertNewsTSpentInMongo(newsTSpentMap_filter)
insertNewsTScoreInMongo(newsTSpentMap_filter)
insert_time = (time.time() - insert_start)
